...
class AglareSetup(ConfigListScreen, Screen):
    ...
    def createSetup(self):
        try:
            ...
            current_posterx = config.plugins.Aglare.InfobarPosterx.value
            if current_posterx not in [v for v, _ in posterx_choices]:
                config.plugins.Aglare.InfobarPosterx.value = 'infobar_posters_posterx_off'
            ...
            # Ensure current value is valid
            current_xtraevent = config.plugins.Aglare.InfobarXtraevent.value
            if current_xtraevent not in [v for v, _ in xtraevent_choices]:
                config.plugins.Aglare.InfobarXtraevent.value = 'infobar_posters_xtraevent_off'
